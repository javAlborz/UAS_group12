function [ route ] = astar_3d( map, start, end_, length_cost )
    ...
    while ~(parent_node.position(1) == end_(1) && ...
            ...
            parent_node.position(3) == end_(3)) %z
    ...
    if first_closed == 1
        ...
        lowest_child_i = -1;
        for child_i = 1:length(children)
            if children(child_i).f + length_cost < lowest_f %only difference made when converting to A*
    ...
    while  ~(parent_node.position(1) == start(1) && ...
             ...
             parent_node.position(3) == start(3)) %Z
